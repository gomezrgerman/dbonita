...
type TranslatedTreatment = typeof TRATAMIENTOS_DESTACADOS[number] & {
...
function MobileCarousel({ items, bookLabel, featuredLabel }: {
  items: TranslatedTreatment[]
  bookLabel: string
  ...
  const onScroll = () => {
    ...
    setActiveIndex(
      Math.min(Math.round(el.scrollLeft / cardStep), items.length - 1)
    )
  }
        ...
        {items.map((item) => (
          ...
          >
            ...
            <div className="absolute inset-x-0 bottom-0 p-5 flex flex-col gap-3 z-10">
              <div>
                <span className="label-upper text-white/50 block mb-1.5">{featuredLabel}</span>
                <h3
              ...
              >
                ...
                <button
                  ...
                  aria-label={`${bookLabel} ${item.translatedName}`}
                >
                  {bookLabel}
...
function TreatmentHoverCard({ item, bookLabel }: { item: TranslatedTreatment; bookLabel: string }) {
  return (
    <motion.div
      custom={0}
      initial="hidden"
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3 z-10">
        <div className="flex items-end justify-between">
          ...
          <span
            ...
            style={{ fontWeight: 800, color: item.color, letterSpacing: '-0.04em' }}
          >
            {item.precio}
          </span>
        </div>
        ...
        >
          ...
          <div className="flex items-center justify-between">
            <span
              ...
              style={{ fontWeight: 800, color: item.color, letterSpacing: '-0.03em' }}
            >
              {item.precio}
            </span>
            <button
              ...
              style={{ fontWeight: 600, background: item.color }}
              onMouseEnter={(e) => {
...
export default function FeaturedTreatments() {
  ...
  const translatedItems: TranslatedTreatment[] = TRATAMIENTOS_DESTACADOS.map((item, i) => ({
    ...item,
    translatedName: t.featuredTreatments.names[i] ?? item.nombre,
    translatedDesc: t.featuredTreatments.descriptions[i] ?? item.descripcion,
  }))
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 pt-12 md:pt-16 pb-6 md:pb-8">
        <div className="flex flex-col gap-4">
          ...
          >
            {t.featuredTreatments.headlinePre}{' '}
            <span style={{ color: 'var(--color-pomegranate)' }}>{t.featuredTreatments.headlineAccent}</span>
          </motion.h2>
        </div>
      </div>
      ...
      <div className="md:hidden">
        <MobileCarousel
          ...
          bookLabel={t.featuredTreatments.book}
          featuredLabel={t.featuredTreatments.featured}
        />
      </div>
      ...
      <div className="hidden md:block max-w-7xl mx-auto px-6 lg:px-12 pb-12 md:pb-16">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {translatedItems.map((item) => (
            <TreatmentHoverCard key={item.id} item={item} bookLabel={t.featuredTreatments.book} />
          ))}
        </div>
      </div>
    </section>
